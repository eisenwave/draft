...
whose type satisfies \exposconcept{queryable}
such that \tcode{o2.query(get_start_scheduler)} is a prvalue
...
if constexpr (@\libconcept{scheduler}@<decltype(data)>) {
  auto orig_sch =
    @\exposid{query-with-default}@(get_start_scheduler, env, @\exposid{not-a-scheduler}@());
...
\tcode{completion_signatures_of_t<\exposid{run-\linebreak{}loop-sender}, E>} is
\begin{codeblock}
...
\tcode{\exposid{single-sender-\brk{}value-type}<Expr, env_of_t<Promise>>}
is well-formed,
...
if constexpr (requires{ std::forward_like<Sndr>(child).affine_on(); })
  ...
  return continues_on(std::forward_like<Sndr>(child),
                      @\exposid{UNSTOPPABLE-SCHEDULER}@(get_start_scheduler(ev)));
\end{codeblock}
...
otherwise returns \tcode{as_awaitable(affine_on(std::forward<Sender>(sndr)), *this)}.
\end{itemdescr}
...
\begin{itemize}
\item \tcode{env.query(get_start_scheduler)} returns \tcode{start_scheduler_type(\exposid{SCHED}(*this))}.
